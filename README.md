...
VDDQ 经 BMC/IPMI（ipmitool）读电压传感器，为整个内存子系统的单一轨压。SMBIOS 的 Configured Voltage 是 JEDEC 标称值（DDR5 恒 1.1 V），不反映 EXPO/XMP 实际设定，故不予显示。装有 ryzen_smu 时 `sckoc vid` 的 `VDDIO_MEM` 亦为内存接口电压来源。
