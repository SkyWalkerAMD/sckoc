...
[ -e /dev/cpu/0/msr ] || { echo "先: sudo modprobe msr"; exit 1; }
...
percore(){
  ...
  for c in $CPUS; do
    ...
    if [ "$VEN" = GenuineIntel ]; then
      ...
      extra="  $(printf '%6s' "$(wt "$dE/2^$eu/$INT")") W  C0 $(printf '%3d' "$c0m")%"
    fi
...
case "${1:-mon}" in
  ...
  *) echo "Usage: msr-sck [mon|vcore] | dump <reg> [hi:lo] | -V | INT=<sec> msr-sck"; exit 1 ;;
esac
MSR_SH
